...
const handleDelete = (row) => {
   ...
   MessageBox.confirm('是否确认删除角色编号为"' + roleIds + '"的数据项?', "警告", {
...
/**导出 */
const handleExport = () => {
   MessageBox.confirm('是否确认导出所有角色数据项?', "警告", {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning"
   }).then(function () {
...
const handleStatusChange = (row) => {
   ...
   MessageBox.confirm('确认要"' + text + '""' + row.roleName + '"角色吗?', "警告", {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning"
   }).then(function () {
...
const handleUpdate = (row) => {
   ...
   getRole(roleId).then(response => {
      ...
      operForm.value.roleSort = Number(operForm.value.roleSort)
      open.value = true;
      nextTick(() => {
         roleMenu.then(res => {
            //获取选中的checkedKeys
   ...
   getRole(row.roleId).then(response => {
      ...
      nextTick(() => {
         roleDeptTreeselect.then(res => {
            nextTick(() => {
               console.log(deptRef.value, '---=----deptRef.value')
               if (deptRef.value) {
